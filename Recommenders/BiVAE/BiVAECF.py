...
from .Recommender import MEASURE_DOT
    ...
    * Quoc-Tuan Truong, Aghiles Salah, Hady W. Lauw. " Bilateral Variational Autoencoder for Collaborative Filtering."
    ...
    def score(self, user_idx, item_idx=None):
        ...
        if self.is_unknown_user(user_idx):
            raise Exception("Can't make score prediction for user %d" % user_idx)

        if item_idx is not None and self.is_unknown_item(item_idx):
            raise Exception("Can't make score prediction for item %d" % item_idx)
